...
fn main() -> Result<(), CoppError> {
    ...
    // 5) Post-process TOPP3-SOCP results: (a,b,s) -> t(s) -> s(t)
    ...
    // (a_qp1, b_qp1) is a feasible but possibly suboptimal profile for TOPP3. It can be directly used for control or as a reference for further optimization unless a more optimal profile is required.
    ...
    // 8) Post-process TOPP3-SOCP results: (a,b,s) -> t(s) -> s(t)
    // t_final is the traversal time of the path.
    // t_s[i] is the time at which the path parameter s_i is reached.
    ...
    // (a_qp2, b_qp2) is a less conservative and more optimal profile for TOPP3 compared with (a_qp1, b_qp1).
